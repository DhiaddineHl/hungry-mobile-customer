...
logoUrl            string            (absolute MinIO/S3 URL)
days               WorkingDay[]
enabled            boolean
```
...
Of those, the backend supplies **`name` only**. It also has no `bannerImage` — just a
single `logoUrl`. Meanwhile the backend supplies address, coordinates, contact, opening
hours, cuisines and accessibility that the UI never shows.
...
- **Not backed by any backend field** — `rating`, `reviewCount`, `deliveryTime`,
  ...
  `isTopRated`, `bannerImage`. These become **optional** on the view-model with a single
  documented constant listing them, so the day an orders/ratings service ships, the
  compiler points at every site to fill in.
...
| `components/home/open-restaurants.tsx` | Drop `RESTAURANTS`, accept data via props |
...
| `components/restaurant/restaurant-header.tsx` | Make unbacked props optional |
...
3. View-model mapping — cuisines joined and stably ordered, missing `logoUrl`, absent
   unbacked fields staying absent rather than becoming `""`.
4. Service layer — query-string construction for filter/sort/page, and 500-on-missing-id
   mapped to a not-found result.
5. Component smoke tests — cards render with sparse data and omit unbacked badges.

## 7. Sequencing

1. Test harness (nothing else is verifiable without it)
2. Schemas + inferred types
3. View model + opening hours (+ their tests — the bulk of the value)
4. Service layer (+ tests)
5. Query keys + hooks
6. Home screen wiring
7. Detail + info screens
8. Favorites store narrowing
